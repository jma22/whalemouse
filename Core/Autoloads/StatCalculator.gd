...
static func get_movement_speed_flat() -> float:
	return GlobalStats.current_run_stats["movement_speed_flat"] * 0.75
...
static func orbs_on_beluga_kill() -> int:
	return GlobalStats.current_run_stats["on_beluga_kill_orb_drop"]

static func on_beluga_kill_cd_refund_percent() -> float:
	return 1 - 0.8 ** GlobalStats.current_run_stats["on_beluga_kill_cd_refund"]
...
static func beluga_freeze_time() -> float:
	if GlobalStats.current_run_stats["beluga_freeze"] == 0:
		return 0.0
	return 2.0 + GlobalStats.current_run_stats["beluga_freeze"] * 1.0

static func get_whale_cooldown_reduction() -> float:
	return  0.9 ** GlobalStats.current_run_stats["whale_cooldown_reduction"] 

static func get_speed_up_multiplier() -> float:
	return 1.1 ** GlobalStats.current_run_stats["movement_speed_up"]

static func get_slow_down_multiplier() -> float:
	return 0.85 ** GlobalStats.current_run_stats["movement_slow_down"]

static func get_dash_cooldown_reduction() -> float:
	return 0.9 ** GlobalStats.current_run_stats["dash_cooldown_reduction"]
...
static func get_dash_distance_decrease() -> float:
	return 0.9 ** GlobalStats.current_run_stats["dash_distance"]

static func has_suicide_dash() -> bool:
	return GlobalStats.current_run_stats["suicide_dash"] > 0

static func get_suicide_dash_cooldown_decrease() -> float:
	return 0.5
...
static func get_attracted_radius() -> float:
	return 0.4 + GlobalStats.current_run_stats["xp_suck"] * 0.4

static func get_attracted_speed() -> float:
	return 0.15 + GlobalStats.current_run_stats["xp_suck"] * 0.4
...
static func get_whale_size() -> float:
	return 0.14 + GlobalStats.current_run_stats["whale_size"] * 0.02 ## previously 0.14

static func get_attack_speed_multiplier() -> float:
	return 1.0 + GlobalStats.current_run_stats["player_attack_speed"] * 0.15
...
static func get_bigger_attack_every_n_hits() -> int:
	if GlobalStats.current_run_stats["bigger_attack_every_n_hits"] == 0:
		return 0
	return max(2,6-GlobalStats.current_run_stats["bigger_attack_every_n_hits"])
...
static func get_bomb_tick_time() -> float:
	return 2.5 * (0.9 ** GlobalStats.current_run_stats["bomb_tick_time"])
...
static func get_bomb_orb_drop_chance() -> float:
	if GlobalStats.current_run_stats["bomb_orb_drop"] == 0:
		return 0.0
	return 0.3 + GlobalStats.current_run_stats["bomb_orb_drop"] * 0.1
...
static func get_poisoned_enemies_drop_ebbs() -> int:
	return GlobalStats.current_run_stats["poisoned_enemies_drop_ebbs"]

static func get_poison_kills_drop_orbs() -> int:
	return GlobalStats.current_run_stats["poison_kills_drop_orbs"]

static func get_poison_tick_multiplier() -> float:
	var m : float = 1.0 - GlobalStats.current_run_stats["faster_poison"] * 0.1
	m *= pow(1.5, GlobalStats.current_run_stats["slower_poison_more_lethal"])
	return max(m, 0.1)

static func get_poison_expiry_damage() -> int:
	return 2 if GlobalStats.current_run_stats["slower_poison_more_lethal"] > 0 else 1
...
static func marks_needed_to_make_bomb() -> int:
	if GlobalStats.current_run_stats["mark_makes_a_bomb"] == 0:
		return 0
	return max(1, 4 - GlobalStats.current_run_stats["mark_makes_a_bomb"])

static func mark_to_orb() -> bool:
	return GlobalStats.current_run_stats["mark_to_orb"] > 0

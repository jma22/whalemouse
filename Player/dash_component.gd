...
func set_dash_cooldown() -> void:
	dash_cooldown_timer = dash_base_cooldown * StatCalculator.get_dash_cooldown_reduction()
...
func get_cooldown_progress() -> float:
	if dash_cooldown_timer <= 0.0:
		return 1.0
	return 1.0 - (dash_cooldown_timer / (dash_base_cooldown * StatCalculator.get_dash_cooldown_reduction()))
...
const BASE_DASH_DISTANCE : float = 10.0
const BASE_GRAPPLE_SPEED : float = 20.0
const CONTACT_THRESHOLD : float = 0.4
...
func fixed_run(_delta: float) -> void:
	if not contacted:
		if grapple_target != null and entity.global_transform.origin.distance_to(grapple_target.global_transform.origin) < CONTACT_THRESHOLD:
			contacted = true
			residual_velocity()
		else:
			return
...
func initial_velocity() -> void:
	if grapple_target != null:
		var raw_diff := grapple_target.global_transform.origin - entity.global_transform.origin
		grapple_direction = Vector3(raw_diff.x, 0, raw_diff.z / Constants.VERTICAL_PERSRPECTIVE_SCALE).normalized()
		entity.velocity.x = grapple_direction.x * BASE_GRAPPLE_SPEED * StatCalculator.get_dash_distance_decrease()
		entity.velocity.z = grapple_direction.z * BASE_GRAPPLE_SPEED * StatCalculator.get_dash_distance_decrease() * Constants.VERTICAL_PERSRPECTIVE_SCALE


func residual_velocity() -> void:
	entity.velocity.x = grapple_direction.x * BASE_DASH_DISTANCE * StatCalculator.get_dash_distance_decrease()
	entity.velocity.z = grapple_direction.z * BASE_DASH_DISTANCE * StatCalculator.get_dash_distance_decrease() * Constants.VERTICAL_PERSRPECTIVE_SCALE

...
static func _static_init() -> void:
	_add_generic_blessing("heal","Time on a Jar", _heal_desc, -3, [_scaled_heal_effect, _increase_stats_effect.bind("heal")])
	_add_one_time_blessing("xp_suck", "Orb Catcher", _xp_suck_desc, -3, [_increase_stats_effect.bind("xp_suck")])
	_add_generic_blessing("enemy_xp_drop", "Feast Totem", _enemy_xp_drop_desc, -3, [_increase_stats_effect.bind("enemy_xp_drop")])
	_add_whale_blessing("whale_level", "Beluga Plushie", _whale_desc, -3, [_increase_stats_effect.bind("whale_level")])
	_add_whale_blessing("whale_cooldown", "Beluga's Boon", _whale_cooldown_desc, -3, [_increase_stats_effect.bind("whale_cooldown")])
	_add_whale_blessing("whale_damage", "Beluga's Boon", _whale_damage_desc, -3, [_increase_stats_effect.bind("whale_damage")])
	_add_generic_blessing("dash_distance", "VROOM!!", _dash_desc, -3,[_increase_stats_effect.bind("dash_distance")])
	...
	_add_curse("enemy_attack_speed", "Flying Shell", _enemy_attack_speed_desc, 10, [_increase_stats_effect.bind("enemy_attack_speed")])
	...
	_add_generic_blessing("attack_size", "Giant Clam", _attack_size_desc, -3, [_increase_stats_effect.bind("attack_size")])
	_add_generic_blessing("player_attack_speed", "Sonic Seashell", _attack_speed_desc, -3, [_increase_stats_effect.bind("player_attack_speed")])
	_add_generic_blessing("ebb_drop", "Ebb Essence", _ebb_drop_desc, -3, [_increase_stats_effect.bind("ebb_drop")])
	_add_one_time_blessing("ebb_on_stand", "Ebb's Embrace", _ebb_on_stand_desc, -3, [_increase_stats_effect.bind("ebb_on_stand")])
	_add_one_time_blessing("damaging_dash", "Shark Teeth", _damaging_dash_desc, -3,[_increase_stats_effect.bind("damaging_dash")])
	_add_generic_blessing("damage_reduction", "Big Shell", _damge_reduction_desc, -3,[_increase_stats_effect.bind("damage_reduction")])
	_add_one_time_blessing("thornmail", "Thornmail", _thornmail_desc, -3, [_increase_stats_effect.bind("thornmail")])
	_add_generic_blessing("fast_while_status", "Swift Current", _fast_during_status_desc, -3, [_increase_stats_effect.bind("fast_while_status")])
	_add_generic_blessing("flat_speed", "Streamlined Shell", _flat_speed_desc, -3, [_increase_stats_effect.bind("flat_speed")])
...
static func scale_curse_damage(amount : int) -> int:
	var damage_scaling : float = 0.5
	var seconds_scaling : float = 2
	var num : float = (10 - amount) * (1  / GlobalStats.get_seconds_per_damage()) * seconds_scaling
	num *= damage_scaling
	num += randi_range(-2, 2)
	num = max(0, num)
	return floor(num)
...
static func get_harder_curse(upgrade_data : UpgradeData) -> UpgradeData:
	...
	new_effects.append(_scaled_damage_effect)

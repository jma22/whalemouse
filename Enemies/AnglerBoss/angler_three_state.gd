...
var num_enemies_to_spawn : int = 5
var pillar_tracker : int = 0
...
func enter() -> void:
	...
	eye.set_variant(0)
	var eye2 : Node3D = entity.enemy_spawner.spawn_enemy("AnglerEye", spawn_position)
	entity.link_health(eye2)
	eye2.set_variant(1)
...
func check_state() -> void:
	...
	if child_state.is_complete:
		if child_state == enemy_spawn_state:
			idle_state.set_idle_duration(4.0)
			state_machine.set_state(idle_state)
		elif child_state == idle_state:
			if randf() < 0.5:
				enemy_spawn_state.set_spawn_info("AnglerPillar", sample_positions_to_pillar())
			else:
...
func sample_positions_to_pillar() -> Array[Vector3]:
	...
	# return positions
	positions.append(entity.pillar_spawn_points[pillar_tracker % entity.pillar_spawn_points.size()])
	pillar_tracker += 1	
	return positions
